...
searchBtn.addEventListener('click', () => {
  searchOnClick('./pages/search/index.html');
});
...
elements.profileBtn.addEventListener('click', () => {
  //  await getUser(token)
  const url = './pages/profile/index.html';
  window.location = `${url}`;
});
...
function forwordToForgotPasswordPage() {
  const url = './pages/forgot-password/index.html';
  window.location = `${url}`;
}

window.openSignUModal = openSignUModal;
window.toggleSignModals = toggleSignModals;
window.forwordToForgotPasswordPage = forwordToForgotPasswordPage;
